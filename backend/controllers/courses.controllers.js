...
export default {
  ...
  allCoursesTypeStart: async (req, res) => {
    try {
      ...
      const result = await genericMongoCrud.getOne(process.env.COLL_COURSES, {
        type,
      });
      res.json(result);
    } finally {
  ...
  oneCoursesFavorites: async (req, res) => {
    try {
      ...
      if (course.length === 0) {
        res
          .status(400)
          .json({ message: "No existe un curso con la ID introducida" });
      } else {
      ...
      res
        .status(400)
        .json({
          mesagge: "Error inesperado al obtener los datos del curso",
          error: e,
        });
    }
  },
